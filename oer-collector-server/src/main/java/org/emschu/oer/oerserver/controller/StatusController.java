...
public class StatusController {
    ...
    @GetMapping("/program/tomorrow")
    public ResponseEntity<List<ProgramEntry>> allTomorrowProgramEntries() {
        return new ResponseEntity<>(programService.getCompleteDailyProgram(LocalDate.now().plus(1, ChronoUnit.DAYS)),
                HttpStatus.ACCEPTED);
    }
    ...
    @GetMapping("/program/tomorrow/{channelId}")
    public ResponseEntity<List<ProgramEntry>> tomorrowProgramEntriesByChannel(@PathVariable Long channelId) {
        if (channelId == null) {
            return ResponseEntity.badRequest().build();
        }
        return new ResponseEntity<>(programService.getDayProgramByChannelId(LocalDate.now().plus(1, ChronoUnit.DAYS), channelId),
                HttpStatus.ACCEPTED);
    }
